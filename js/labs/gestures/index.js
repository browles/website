...
App.prototype.grid = function(n) {
    ...
    for(var i = 0; i < n; i++) {
        ...
        panel.setProportionalSize(p - 0.001,0.9 * p - 0.001,0);
        panel.setAlign((i % sqrt) * p, 0.1 + 0.9*Math.floor(i / sqrt) * p,0);
        ...
        bodies.push(b);
        var spring = new physics.Spring(null,b, {period:0.75, dampingRatio:1, anchor: new math.Vec3()});
        var rspring = new physics.RotationalSpring(null,b, {period:0.75, dampingRatio:0.5});
...
function BoxView(node, options) {
    ...
    this.el.setProperty('borderRadius', '1px');
